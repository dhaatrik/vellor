...
interface FriendlyTutorAdviceModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const FriendlyTutorAdviceModal: React.FC<FriendlyTutorAdviceModalProps> = ({ isOpen, onClose }) => {
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
        ...
        >
          <div className="bg-gray-50 dark:bg-primary-light p-6 md:p-8 border-b border-gray-100 dark:border-white/5 flex items-center justify-between shrink-0">
            ...
            <button onClick={() => onClose()} className="p-2 bg-gray-200 dark:bg-white/10 hover:bg-gray-300 dark:hover:bg-white/20 rounded-full transition-colors self-start" aria-label="Close Friendly Tutor Advice" title="Close Friendly Tutor Advice">
          ...
          <div className="p-6 md:p-8 space-y-6 text-gray-600 dark:text-gray-300 text-base md:text-lg overflow-y-auto custom-scrollbar">
            ...
            <div className="pt-8 text-center pb-4">
              <Button onClick={() => onClose()} className="px-10 py-4 text-lg rounded-full font-bold shadow-lg shadow-accent/20">Let's Get Back to Teaching</Button>
            </div>
          </div>
        </motion.div>
      </div>
